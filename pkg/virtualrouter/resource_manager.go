...
func NewDefaultResourceManager(k8sClient client.Client, appMeshSDK services.AppMesh, meshRefResolver mesh.ReferenceResolver,
	vnRefResolver ref.ReferenceResolver, accountID string, log logr.Logger) ResourceManager {
...
func (m *defaultResourceManager) Reconcile(ctx context.Context, vr *appmesh.VirtualRouter) error {
	...
	vnByRefHash, err := m.findVirtualNodeDependencies(ctx, vr)
	if err != nil {
		return err
	}
	...
	if sdkVR == nil {
		...
		sdkRouteByName, err = m.routesManager.create(ctx, ms, vr, vnByRefHash)
		if err != nil {
			return err
		}
	} else {
		...
		sdkRouteByName, err = m.routesManager.update(ctx, ms, vr, vnByRefHash)
		if err != nil {
			return err
		}
	}
...
func (m *defaultResourceManager) findMeshDependency(ctx context.Context, vr *appmesh.VirtualRouter) (*appmesh.Mesh, error) {
	...
	ms, err := m.meshRefResolver.Resolve(ctx, *vr.Spec.MeshRef)
	if err != nil {
...
func (m *defaultResourceManager) findVirtualNodeDependencies(ctx context.Context, vr *appmesh.VirtualRouter) (map[vnReferenceHash]*appmesh.VirtualNode, error) {
	vnRefs := extractVirtualNodeReferences(vr)
	vnByRefHash := make(map[vnReferenceHash]*appmesh.VirtualNode, len(vnRefs))
	for _, vnRef := range vnRefs {
		vnRefHash := virtualNodeReferenceToHash(vnRef)
		vn, err := m.vnRefResolver.Resolve(ctx, vr, vnRef)
		if err != nil {
			return nil, errors.Wrapf(err, "failed to resolve virtualNodeRef")
		}
		vnByRefHash[vnRefHash] = vn
	}
	return vnByRefHash, nil
}
