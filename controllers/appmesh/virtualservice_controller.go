...
func NewVirtualServiceReconciler(k8sClient client.Client, finalizerManager k8s.FinalizerManager, vsResourceManager virtualservice.ResourceManager, log logr.Logger) *virtualServiceReconciler {
...
type virtualServiceReconciler struct {
	...
	vsResourceManager virtualservice.ResourceManager
	log               logr.Logger
}
...
func (r *virtualServiceReconciler) reconcileVirtualService(ctx context.Context, vs *appmesh.VirtualService) error {
	...
	if err := r.vsResourceManager.Reconcile(ctx, vs); err != nil {
		return err
	}
	return nil
}
...
func (r *virtualServiceReconciler) cleanupVirtualService(ctx context.Context, vs *appmesh.VirtualService) error {
	if k8s.HasFinalizer(vs, k8s.FinalizerAWSAppMeshResources) {
		if err := r.vsResourceManager.Cleanup(ctx, vs); err != nil {
			return err
		}
